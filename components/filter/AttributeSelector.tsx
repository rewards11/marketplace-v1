  ...
  return (
    <div className="border-b-[1px] border-gray-300 dark:border-neutral-600">
      <div className="relative m-4">
        ...
        />
        <DebounceInput
          className="deb_input reservoir-label-l input-primary-outline w-full border-[#D1D5DB] pl-9 focus:border-[#D1D5DB] dark:border-neutral-600 dark:bg-neutral-900 dark:text-white dark:ring-primary-900 dark:focus:ring-4"
          type="text"
          autoFocus
          value={query}
          autoComplete="off"
      ...
      <div className="mb-3 max-h-[155px] overflow-y-auto">
        {searchedValues.map(({ value, count }, index) => {
          ...
          return (
            ...
            >
              <span className="reservoir-body hover:text-[#cc00ff] text-white">{value}</span>
              <span className="reservoir-body hover:text-[#cc00ff] text-white">{count}</span>
            </AttributeButton>
          )
        })}
      </div>
