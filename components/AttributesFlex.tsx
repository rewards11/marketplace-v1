  ...
  return (
    <div className={className}>
      {filters.map(({ key, value }) => ( 
        <div
          key={key}
          className="nft__item_ws hidden rounded-full border border-neutral-300 bg-primary-100 px-4 py-3 dark:border-neutral-600 dark:bg-primary-900 dark:text-white md:flex"
        >
          <div className="reservoir-label-l flex items-center justify-between gap-1.5 dark:text-white ">
            <p className="fn_title_ws capitalize">{key} - </p>
            <p className="fn_title_ws_1"> {value}</p>
          </div>
          <button
            className="ml-4"
            onClick={() => toggleOffAttribute(router, key)}
          >
            <HiX className="img_holder_ws h-3.5 w-3.5" />
          </button>
        </div>
      ))}
    </div>
